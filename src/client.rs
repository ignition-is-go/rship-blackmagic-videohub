...
impl Default for VideohubState {
    fn default() -> Self {
        Self {
            device_info: None,
            input_labels: HashMap::new(),
            output_labels: HashMap::new(),
            video_output_routing: HashMap::new(),
            connected: false,
        }
    }
}
...
impl VideohubClient {
    ...
    pub async fn connect(&mut self) -> Result<()> {
        ...
        let framed = Framed::new(stream, VideohubCodec::default());
    ...
    pub async fn send_message(&mut self, message: VideohubMessage) -> Result<()> {
        if let Some(conn) = &mut self.connection {
            conn.send(message).await.map_err(|e| anyhow!("Failed to send message: {}", e))?;
            Ok(())
        } else {
    ...
    fn handle_message(&mut self, message: &VideohubMessage) {
        match message {
            ...
            VideohubMessage::OutputLabels(labels) => {
                ...
                for label in labels {
                    self.state.output_labels.insert(label.id, label.name.clone());
                }
            }
            VideohubMessage::VideoOutputRouting(routes) => {
                ...
                for route in routes {
                    self.state.video_output_routing.insert(route.to_output, route.from_input);
                }
            }
            VideohubMessage::ACK => {
